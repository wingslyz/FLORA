...
def extend_cfg(cfg, args):
    ...
    cfg.TRAINER.FLORA.N_CTX = args.n_ctx  # number of context vectors
    cfg.TRAINER.FLORA.CSC = False  # class-specific context
    cfg.TRAINER.FLORA.CTX_INIT = args.ctx_init  # initialization words
    cfg.TRAINER.FLORA.PREC = "fp32"  # fp16, fp32, amp
    cfg.TRAINER.FLORA.CLASS_TOKEN_POSITION = "end"  # 'middle' or 'end' or 'front'
    cfg.TRAINER.FLORA.BOTTLENECK = args.bottleneck
    cfg.TRAINER.FLORA.N = args.num_prompt # number of prompts
    cfg.TRAINER.FLORA.FEATURE = args.feature
    cfg.TRAINER.FLORA.mu = args.mu
    cfg.TRAINER.FLORA.oo = args.oo
    cfg.TRAINER.FLORA.temp = args.temp
...
def main(args):
    ...
    local_weights_adapter = [[] for i in range(args.num_users)]
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if args.trainer == 'CLIP':
            ...
            for idx in idxs_users:
                ...
                local_weights_3[idx] = copy.deepcopy(local_weight['prompt_learner.ctx_local'])

                adapter_params = {}
                for param_name in ['image_adapter.0.weight', 'image_adapter.0.bias',
                                   'image_adapter.2.weight', 'image_adapter.2.bias']:
                    local_params = []
                    for idx in idxs_users:
                        if param_name in local_weights[idx]:
                            local_params.append(copy.deepcopy(local_weights[idx][param_name]))


            print("------------local train finish epoch:", epoch, "-------------")

            glo_sigma = average_weights(local_weights_0,idxs_users,datanumber_client,islist=True)
            if local_params:
                adapter_params[param_name] = average_weights(local_params, idxs_users, datanumber_client,
                                                             islist=True)
            ...
            for idx in all_users:
                ...
                local_weights_per[idx]['prompt_learner.sigma'] = glo_sigma
                local_weights_per[idx]['prompt_learner.B'] = local_weights_1[idx]
                local_weights_per[idx]['prompt_learner.A'] = local_weights_2[idx]
                local_weights_per[idx]['prompt_learner.ctx_local'] = local_weights_3[idx]

                for param_name, param_value in adapter_params.items():
                    local_weights_per[idx][param_name] = param_value
